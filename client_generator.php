...
class CLBlastClientGenerator
{
    ...
    public function parser(string $line) : ?array
    {
        ...
        $pattern = "/^(".$this->preg_types.") PUBLIC_API *([A-Za-z0-9_]+) *\\(([^)]*)/";
        //var_dump($pattern);
    ...
    public function funcTemplate($declare)
    {
        ...
        $code .= ")\n";
        $code .= "{\n";
        $code .= "    if(_h_clblast==NULL || _g_{$funcname}==NULL) {\n";
        if($return=='void') {
            $code .= "        return;\n";
        } else {
            $code .= "        return 0;\n";
    ...
    function beginTemplate()
    {
        ...
        $code .= "#include <clblast_c.h>\n";
        $code .= "\n";
        $code .= "#define LOADFUNC(funcname) \\\n";
        $code .= "_g_##funcname = (PFN##funcname)GetProcAddress( _h_clblast, #funcname ); \\\n";
        $code .= "if(!_g_##funcname) { \\\n";
        $code .= "    printf(\"load error: %s\\n\",  #funcname); \\\n";
        $code .= "    return -1; \\\n";
        $code .= "} \\\n";
        $code .= "\n";
        $code .= "static HMODULE _h_clblast = NULL;\n";
        return $code;
    }
    
    public function endTemplate($funcs)
    {
        $code  = "int rindow_load_clblast_dll()\n";
        $code .= "{\n";
        $code .= "    if(_h_clblast!=NULL) {\n";
        $code .= "        return 0;\n";
        $code .= "    }\n";
        $code .= "    _h_clblast = LoadLibraryA( \"clblast.dll\" );\n";
        $code .= "    if(_h_clblast==NULL) {\n";
        $code .= "        printf(\"load error: clblast\\n\");\n";
        $code .= "        return -1;\n";
        $code .= "    }\n";
        ...
        $code .= "    return 0;\n";
        $code .= "}\n";
        $code .= "void rindow_unload_clblast_dll()\n";
        $code .= "{\n";
        $code .= "    FreeLibrary( _h_clblast );\n";
        $code .= "    _h_clblast = NULL;\n";
        $code .= "}\n";
        return $code;
    }
}

$generator = new CLBlastClientGenerator();
