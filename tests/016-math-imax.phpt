...
try {
    $math->imax(NMITEM,$invalidBuffer,$offsetR=0,$buffer,$offset=0,$inc=1,$queue,$events);
} catch (\Throwable $e) {
    echo "Invalid Buffer catch: ".get_class($e)."\n";
}
try {
    $math->imax(NMITEM,$bufferR,$offsetR=0,$invalidBuffer,$offset=0,$inc=1,$queue,$events);
} catch (\Throwable $e) {
    echo "Invalid Buffer catch: ".get_class($e)."\n";
}
$events = new Rindow\OpenCL\EventList();
...
try {
    $math->imax(NMITEM,$bufferR,$offsetR=0,$buffer,$offset=0,$inc=1,$invalidQueue,$events);
} catch (\Throwable $e) {
...
try {
    $math->imax(NMITEM,$bufferR,$offsetR=0,$buffer,$offset=0,$inc=1,$queue,$invalidEvents);
} catch (\Throwable $e) {
